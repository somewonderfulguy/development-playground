...
import { TeamMappingEntry, TheSportsDBTeamsResponse, VenueMappingEntry } from '../types/teamTypes'
...
export const getVenuesMapping = async () => {
  const { rows: venues } = await sql<VenueMappingEntry>`SELECT * FROM venues`
  return venues
}
